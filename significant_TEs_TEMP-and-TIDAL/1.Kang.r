...
for (f in 1:length(list.files)) {
 		...
 		colnames(x) <- c("Chr", "Start", "End", "TransposonName", "TransposonDirection", "Class", "VariantSupport", "Frequency", "Junction1Support", "Junction2Support", "p5_Support", "p3_Support", "Strain")
 		y <- x
...
### Compare TEMP and TIDAL TE insrtion intervals
...
for (f in 1:length(list.files.temp)) {
	...
	i.tidal <- strsplit(list.files.tidal[f], split=".", fixed=T)[[1]][1]
	if ( i.temp == i.tidal ) {
		...
		# Remove redundancy
		data1.j <- as.data.frame(data1.p) %>% unite(All, c(Chr, Start,  Stop, TE_ID, Reads, Freq), 
								sep = "&", remove = TRUE)
...
list.files.sel  <- dir(parent.o, pattern ="AK.*D_new_intervals_1p1.f0.10_reads.txt") 
list.files.con  <- dir(parent.o, pattern ="AK.*C_new_intervals_1p1.f0.10_reads.txt")
...
for (s in 1:length(list.files.sel)) {
    ...
    name.s <- strsplit(list.files.sel[s], split="_new", fixed=T)[[1]][1]			
	...
	for (f in 1:length(list.files.con)) {										
		...
		data1 <- NULL
		name.c <- strsplit(list.files.con[f], split="_new", fixed=T)[[1]][1]
...
# Selection pool A
list.files.sel  <- dir(parent.o, pattern ="Freqs_AK2_1D-AK2_.*C_f0.10.Pval_BF_reads.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_AK2_1D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
							quote=F, sep="\t", row.names=F, col.names=T)

# Selection pool B
list.files.sel  <- dir(parent.o, pattern ="Freqs_AK2_2D-AK2_.*C_f0.10.Pval_BF_reads.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_AK2_2D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
							quote=F, sep="\t", row.names=F, col.names=T)

# Selection pool C
list.files.sel  <- dir(parent.o, pattern ="Freqs_AK2_3D-AK2_.*C_f0.10.Pval_BF_reads.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_AK2_3D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
							quote=F, sep="\t", row.names=F, col.names=T)
...
saa <- read.table(file=paste(parent.o, "Freqs_AK2_1D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
					header=T, sep="\t")
sbb <- read.table(file=paste(parent.o, "Freqs_AK2_2D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
					header=T, sep="\t")
scc <- read.table(file=paste(parent.o, "Freqs_AK2_3D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), 
					header=T, sep="\t")

colnames(saa) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SELD14",   
					"Stop_SELD14","Start_CTR14","Stop_CTR14","Freq_SELD14","Freq_CTR14", "Pval14","Pval_BF14","Start_SELD17","Stop_SELD17","Start_CTR17",
					"Stop_CTR17","Freq_SELD17","Freq_CTR17","Pval17","Pval_BF17","Start_SELD18","Stop_SELD18","Start_CTR18","Stop_CTR18","Freq_SELD18",
					"Freq_CTR18","Pval18","Pval_BF18")
colnames(sbb) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SELD14",   
					"Stop_SELD14","Start_CTR14","Stop_CTR14","Freq_SELD14","Freq_CTR14", "Pval14","Pval_BF14","Start_SELD17","Stop_SELD17","Start_CTR17",
					"Stop_CTR17","Freq_SELD17","Freq_CTR17","Pval17","Pval_BF17","Start_SELD18","Stop_SELD18","Start_CTR18","Stop_CTR18","Freq_SELD18",
					"Freq_CTR18","Pval18","Pval_BF18")
colnames(scc) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SELD14",   
					"Stop_SELD14","Start_CTR14","Stop_CTR14","Freq_SELD14","Freq_CTR14", "Pval14","Pval_BF14","Start_SELD17","Stop_SELD17","Start_CTR17",
					"Stop_CTR17","Freq_SELD17","Freq_CTR17","Pval17","Pval_BF17","Start_SELD18","Stop_SELD18","Start_CTR18","Stop_CTR18","Freq_SELD18",
					"Freq_CTR18","Pval18","Pval_BF18")
...
colnames(saa) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL",   
					"Stop_SEL","Start_CTR","Stop_CTR","Freq_SEL","Freq_CTR", "Pval","Pval_BF","Start_SEL","Stop_SEL","Start_CTR",   
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF","Start_SEL","Stop_SELC","Start_CTR","Stop_CTR","Freq_SEL",  
					"Freq_CTR","Pval","Pval_BF")
colnames(sbb) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL",   
					"Stop_SEL","Start_CTR","Stop_CTR","Freq_SEL","Freq_CTR", "Pval","Pval_BF","Start_SEL","Stop_SEL","Start_CTR",   
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF","Start_SEL","Stop_SELC","Start_CTR","Stop_CTR","Freq_SEL",  
					"Freq_CTR","Pval","Pval_BF")  
colnames(scc) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL",   
					"Stop_SEL","Start_CTR","Stop_CTR","Freq_SEL","Freq_CTR", "Pval","Pval_BF","Start_SEL","Stop_SEL","Start_CTR",   
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF","Start_SEL","Stop_SELC","Start_CTR","Stop_CTR","Freq_SEL",  
					"Freq_CTR","Pval","Pval_BF") 
...
colnames(sa) <- c("Chr","TE_ID","Start_SEL_all","Stop_SEL_all",
					"Start_SEL1D_new","Stop_SEL1D_new","Start_SEL1D","Stop_SEL1D","Start_CTR4C","Stop_CTR4C","Freq_SEL1D",
					"Freq_CTR4C","Pval","Pval_BF","Start_SEL1D","Stop_SEL1D","Start_CTR7C","Stop_CTR7C","Freq_SEL1D",
					"Freq_CTR7C","Pval","Pval_BF","Start_SEL1D","Stop_SEL1D","Start_CTR8C","Stop_CTR8C","Freq_SEL1D",
					"Freq_CTR8C","Pval","Pval_BF","Start_SEL2D_new","Stop_SEL2D_new","Start_SEL2D","Stop_SEL2D","Start_CTR4C",
					"Stop_CTR4C","Freq_SEL2D","Freq_CTR4C","Pval","Pval_BF","Start_SEL2D","Stop_SEL2D","Start_CTR7C","Stop_CTR7C",
					"Freq_SEL2D","Freq_CTR7C","Pval","Pval_BF","Start_SEL2D","Stop_SEL2D","Start_CTR8C","Stop_CTR8C","Freq_SEL2D",
					"Freq_CTR8C","Pval","Pval_BF","Start_SEL3D_new","Stop_SEL3D_new","Start_SEL3D","Stop_SEL3D","Start_CTR4C",
					"Stop_CTR4C","Freq_SEL3D","Freq_CTR4C","Pval","Pval_BF","Start_SEL3D","Stop_SEL3D","Start_CTR7C","Stop_CTR7C",
					"Freq_SEL3D","Freq_CTR7C","Pval","Pval_BF","Start_SEL3D","Stop_SEL3D","Start_CTR8C","Stop_CTR8C","Freq_SEL3D",
					"Freq_CTR8C","Pval","Pval_BF")
...
p.ranges.red <- reduce(p.ranges)
p.new.int <- as.data.frame(p.ranges.red)
r.sel <- cbind(unlist(sapply(strsplit(as.character(p.new.int[,1]),'&&'), "[", 1)), unlist(sapply(strsplit(as.character(p.new.int[,1]),'&&'), 
				"[", 2)), p.new.int[,2], p.new.int[,3])  
r.sel1 <- as.data.frame(r.sel, stringsAsFactors=FALSE) 
r.sel1[,3] <- as.numeric(r.sel1[,3])
r.sel1[,4] <- as.numeric(r.sel1[,4])
colnames(r.sel1) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new")
...
colnames(sa) <- c("Chr","TE_ID","Start_SEL_all","Stop_SEL_all","Start_SEL1D_new","Stop_SEL1D_new","Start_SEL1D",
					"Stop_SEL1D","Start_CTR4C","Stop_CTR4C","Freq_SEL1D","Freq_CTR4C","Pval","Pval_BF","Start_SEL1D",
					"Stop_SEL1D","Start_CTR7C","Stop_CTR7C","Freq_SEL1D","Freq_CTR7C","Pval","Pval_BF","Start_SEL1D",
					"Stop_SEL1D","Start_CTR8C","Stop_CTR8C","Freq_SEL1D","Freq_CTR8C","Pval","Pval_BF","Start_SEL2D_new",
					"Stop_SEL2D_new","Start_SEL2D","Stop_SEL2D","Start_CTR4C","Stop_CTR4C","Freq_SEL2D","Freq_CTR4C",
					"Pval","Pval_BF","Start_SEL2D","Stop_SEL2D","Start_CTR7C","Stop_CTR7C","Freq_SEL2D","Freq_CTR7C",
					"Pval","Pval_BF","Start_SEL2D","Stop_SEL2D","Start_CTR8C","Stop_CTR8C","Freq_SEL2D","Freq_CTR8C",
					"Pval","Pval_BF","Start_SEL3D_new","Stop_SEL3D_new","Start_SEL3D","Stop_SEL3D","Start_CTR4C","Stop_CTR4C",
					"Freq_SEL3D","Freq_CTR4C","Pval","Pval_BF","Start_SEL3D","Stop_SEL3D","Start_CTR7C","Stop_CTR7C",
					"Freq_SEL3D","Freq_CTR7C","Pval","Pval_BF","Start_SEL3D","Stop_SEL3D","Start_CTR8C","Stop_CTR8C",
					"Freq_SEL3D","Freq_CTR8C","Pval","Pval_BF")
...
saa <- read.table(file=paste(parent.o, "Freqs_AK2_1D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), header=T, sep="\t", stringsAsFactors = FALSE)
sbb <- read.table(file=paste(parent.o, "Freqs_AK2_2D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), header=T, sep="\t", stringsAsFactors = FALSE)
scc <- read.table(file=paste(parent.o, "Freqs_AK2_3D-3CTRs_f0.10.Pval_BF_reads.txt", sep=""), header=T, sep="\t", stringsAsFactors = FALSE)
...
write.table(saa.i, file=paste(parent.o, "One_sign_1D_increase_freqs_in_other_pools_one_sign_1D_f0.10.Pval_BF_reads.txt", sep=""), 
			quote=F, col.names=T, row.names=F, sep="\t")
