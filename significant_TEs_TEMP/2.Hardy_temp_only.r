...
for (d in 1:length(list.files)) {
	...
	# Include the name of the strain for each ipnut file (ex: CA, CB...)	
...
for (f in 1:length(list.files.temp)) {
	...
	write.table(temp.f, file=paste(parent.o, name, ".insertion.1p1.f0.10_ref.txt", sep=""), 
				quote=F, sep="\t", row.names=F, col.names=T)
}

list.files.sel  <- dir(parent.o, pattern ="S.*insertion.1p1.f0.10_ref.txt") 
list.files.con  <- dir(parent.o, pattern ="C.*insertion.1p1.f0.10_ref.txt")
				...
				if (input.s.n[i,1] == input.c.n[j,1]) {
					if (c(input.s.n[i,2],input.s.n[i,3]) %overlaps% c(input.c.n[j,2], input.c.n[j,3])) {
						pval <- fisher.test(matrix(c(as.numeric(input.s.n[i,6]), as.numeric(input.s.n[i,13]), 
											as.numeric(input.c.n[j,6]), as.numeric(input.c.n[j,13])), 
											nrow=2, ncol=2, byrow=T))$p.val
						data1 <- rbind(data1, c(strsplit(input.s.n[i,1], split="&&")[[1]][1], strsplit(input.s.n[i,1], split="&&")[[1]][2], input.s.n[i,2], input.s.n[i,3], input.c.n[j,2], input.c.n[j,3], input.s.n[i,7], input.c.n[j,7], scientific(as.numeric(pval), digits=2)))
					}
				}		
			}
		}
		# P-values
		colnames(data1) <- c("Chr", "TE_ID", "Start_SEL", "Stop_SEL", "Start_CTR", "Stop_CTR", "Freq_SEL", "Freq_CTR",
							 "Pval")
...
# Selection pool A
list.files.sel  <- dir(parent.o, pattern ="Freqs_SA-C.*_f0.10.Pval_BF_reads_temp.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_SA-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
			quote=F, sep="\t", row.names=F, col.names=T)

# Selection pool B
list.files.sel  <- dir(parent.o, pattern ="Freqs_SB-C.*_f0.10.Pval_BF_reads_temp.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_SB-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
							quote=F, sep="\t", row.names=F, col.names=T)

# Selection pool C
list.files.sel  <- dir(parent.o, pattern ="Freqs_SC-C.*_f0.10.Pval_BF_reads_temp.txt")
...
write.table(sa, file=paste(parent.o, "Freqs_SC-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
			quote=F, sep="\t", row.names=F, col.names=T)
...
saa <- read.table(file=paste(parent.o, "Freqs_SA-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), header=T, sep="\t")
sbb <- read.table(file=paste(parent.o, "Freqs_SB-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), header=T, sep="\t")
scc <- read.table(file=paste(parent.o, "Freqs_SC-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), header=T, sep="\t")

colnames(saa) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL11","Stop_SEL11","Start_CTR1",
					"Stop_CTR1","Freq_SEL11","Freq_CTR1", "Pval11","Pval_BF11","Start_SEL12","Stop_SEL12",
					"Start_CTR2","Stop_CTR2","Freq_SEL12","Freq_CTR2","Pval12","Pval_BF12","Start_SEL13",
					"Stop_SEL13","Start_CTR3","Stop_CTR3","Freq_SEL13","Freq_CTR3","Pval13","Pval_BF13")
colnames(sbb) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL11","Stop_SEL11","Start_CTR1",
					"Stop_CTR1","Freq_SEL11","Freq_CTR1", "Pval11","Pval_BF11","Start_SEL12","Stop_SEL12",
					"Start_CTR2","Stop_CTR2","Freq_SEL12","Freq_CTR2","Pval12","Pval_BF12","Start_SEL13",
					"Stop_SEL13","Start_CTR3","Stop_CTR3","Freq_SEL13","Freq_CTR3","Pval13","Pval_BF13")
colnames(scc) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL11","Stop_SEL11","Start_CTR1",
					"Stop_CTR1","Freq_SEL11","Freq_CTR1", "Pval11","Pval_BF11","Start_SEL12","Stop_SEL12",
					"Start_CTR2","Stop_CTR2","Freq_SEL12","Freq_CTR2","Pval12","Pval_BF12","Start_SEL13",
					"Stop_SEL13","Start_CTR3","Stop_CTR3","Freq_SEL13","Freq_CTR3","Pval13","Pval_BF13")
...
# Significant tin selection pool and in one, two or three controls and increase in frequency from control to selection pool
...
colnames(sbb) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL","Stop_SEL","Start_CTR",
					...
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF","Start_SEL","Stop_SELC","Start_CTR",
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF")  
colnames(scc) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL","Stop_SEL","Start_CTR",
					"Stop_CTR","Freq_SEL","Freq_CTR", "Pval","Pval_BF","Start_SEL","Stop_SEL","Start_CTR",   
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF","Start_SEL","Stop_SELC","Start_CTR",
					"Stop_CTR","Freq_SEL","Freq_CTR","Pval","Pval_BF") 
...
colnames(sa) <- c("Chr","TE_ID","Start_SEL_all","Stop_SEL_all","Start_SELA_new","Stop_SELA_new",
				"Start_SELA","Stop_SELA","Start_CTRA","Stop_CTRA","Freq_SELA","Freq_CTRA","Pval","Pval_BF",
				"Start_SELA","Stop_SELA","Start_CTRB","Stop_CTRB","Freq_SELA","Freq_CTRB","Pval","Pval_BF",
				"Start_SELA","Stop_SELA","Start_CTRC","Stop_CTRC","Freq_SELA","Freq_CTRC","Pval","Pval_BF",
				"Start_SELB_new","Stop_SELB_new","Start_SELB","Stop_SELB","Start_CTRA","Stop_CTRA",
				"Freq_SELB","Freq_CTRA","Pval","Pval_BF","Start_SELB","Stop_SELB","Start_CTRB","Stop_CTRB",
				"Freq_SELB","Freq_CTRB","Pval","Pval_BF","Start_SELB","Stop_SELB","Start_CTRC","Stop_CTRC",
				"Freq_SELB","Freq_CTRC","Pval","Pval_BF","Start_SELC_new","Stop_SELC_new","Start_SELC",
				"Stop_SELC","Start_CTRA","Stop_CTRA","Freq_SELC","Freq_CTRA","Pval","Pval_BF","Start_SELC",
				"Stop_SELC","Start_CTRB","Stop_CTRB","Freq_SELC","Freq_CTRB","Pval","Pval_BF","Start_SELC",
				"Stop_SELC","Start_CTRC","Stop_CTRC","Freq_SELC","Freq_CTRC","Pval","Pval_BF")
...
#sa[,5] <- as.numeric(sa[,5])
#sa[,31] <- as.numeric(sa[,31])
#sa[,57] <- as.numeric(sa[,57])
...
colnames(sa) <- c("Chr","TE_ID","Start_SEL_all","Stop_SEL_all","Start_SELA_new","Stop_SELA_new",
					...
					"Start_SELA","Stop_SELA","Start_CTRB","Stop_CTRB","Freq_SELA","Freq_CTRB","Pval","Pval_BF",
					"Start_SELA","Stop_SELA","Start_CTRC","Stop_CTRC","Freq_SELA","Freq_CTRC","Pval","Pval_BF",
					"Start_SELB_new","Stop_SELB_new","Start_SELB","Stop_SELB","Start_CTRA","Stop_CTRA","Freq_SELB",
					"Freq_CTRA","Pval","Pval_BF","Start_SELB","Stop_SELB","Start_CTRB","Stop_CTRB","Freq_SELB",
					"Freq_CTRB","Pval","Pval_BF","Start_SELB","Stop_SELB","Start_CTRC","Stop_CTRC","Freq_SELB",
					"Freq_CTRC","Pval","Pval_BF","Start_SELC_new","Stop_SELC_new","Start_SELC","Stop_SELC",
					"Start_CTRA","Stop_CTRA","Freq_SELC","Freq_CTRA","Pval","Pval_BF","Start_SELC","Stop_SELC",
					"Start_CTRB","Stop_CTRB","Freq_SELC","Freq_CTRB","Pval","Pval_BF","Start_SELC","Stop_SELC",
					"Start_CTRC","Stop_CTRC","Freq_SELC","Freq_CTRC","Pval","Pval_BF")
...
saa <- read.table(file=paste(parent.o, "Freqs_SA-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
					header=T, sep="\t", stringsAsFactors = FALSE)
sbb <- read.table(file=paste(parent.o, "Freqs_SB-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
					header=T, sep="\t", stringsAsFactors = FALSE)
scc <- read.table(file=paste(parent.o, "Freqs_SC-3CTRs_f0.10.Pval_BF_reads_temp.txt", sep=""), 
					header=T, sep="\t", stringsAsFactors = FALSE)

oni <- read.table(file=paste(parent.o, "Freqs_1pools_SEL-3CTRs_Pval_f0.10_BF_reads_increase.txt", sep=""), 
					sep="\t", header=T, stringsAsFactors = FALSE)
ond <- read.table(file=paste(parent.o, "Freqs_1pools_SEL-3CTRs_Pval_f0.10_BF_reads_decrease.txt", sep=""), 
					sep="\t", header=T, stringsAsFactors = FALSE)
...
colnames(saa.d) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL","Stop_SEL",
					...
					"Pval.1","Pval_BF.1","Start_SEL.2","Stop_SEL.2","Start_CTRC","Stop_CTRC", 
					"Freq_SEL.2","Freq_CTRC","Pval.2","Pval_BF.2", "other_pool_non_sign")
write.table(saa.d, file=paste(parent.o, "One_sign_SA_decrease_freqs_in_other_pools_f0.10.Pval_BF_reads.txt", sep=""), 
			quote=F, col.names=T, row.names=F, sep="\t")
...
write.table(scc.i, file=paste(parent.o, "One_sign_SC_increase_freqs_in_other_pools_f0.10.Pval_BF_reads.txt", sep=""), 
			quote=F, col.names=T, row.names=F, sep="\t")
...
colnames(scc.d) <- c("Chr","TE_ID","Start_SEL_new","Stop_SEL_new","Start_SEL","Stop_SEL",
					...
					"Pval.1","Pval_BF.1","Start_SEL.2","Stop_SEL.2","Start_CTRC","Stop_CTRC", 
					"Freq_SEL.2","Freq_CTRC","Pval.2","Pval_BF.2", "other_pool_non_sign")
write.table(scc.d, file=paste(parent.o, "One_sign_SC_decrease_freqs_in_other_pools_f0.10.Pval_BF_reads.txt", sep=""), 
			quote=F, col.names=T, row.names=F, sep="\t")
