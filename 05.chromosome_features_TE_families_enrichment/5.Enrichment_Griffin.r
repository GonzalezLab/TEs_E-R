...
## Read files with predicted TEs (the columns are: chr, TE_ID, start, stop, DR1, DR2, DR3, DR4, DR5; the five last columns contain the name of the selection pool if the TE is present or NA if it is absent)
...
# Find unique rows
pool1_tt <- unique(pool1_tt[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
...
# Find unique rows
pool1_t <- unique(pool1_t[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
...
sig_tt <- read.table(file=paste(parent.f, "significant_griffin.txt", sep=""), 
...
sig_t <- read.table(file=paste(parent.f, "only_temp/", "significant_griffin_temp.txt", sep=""), 
					header=F, stringsAsFactors=F)
...
#Find unique rows
pool1_tt <- unique(pool1_tt[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
pool2_tt <- unique(pool2_tt[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
pool3_tt <- unique(pool3_tt[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])

pool1_t <- unique(pool1_t[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
pool2_t <- unique(pool2_t[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
pool3_t <- unique(pool3_t[, c("Chr", "TE_ID","Start","Stop","DR1","DR2","DR3","DR4","DR5")])
...
# Significant TEs predicted with TEMP and TIDAL
sig_tt <- read.table(file=paste(parent.o, "significant_griffin.txt", sep=""), 
						header=F, stringsAsFactors=F)
sig1_tt <- sig_tt[sig_tt[ ,5 ] == 1, ]
sig_many_tt <- sig_tt[sig_tt[ ,5 ] > 1, ]

# Significant TEs predicted with only TEMP
sig_t <- read.table(file=paste(parent.o, "only_temp/", "significant_griffin_temp.txt", sep=""), 
					header=F, stringsAsFactors=F)
...
if (dim(unique(ssm1))[1] == dim(unique(ssm1))[1] | dim(unique(ssm1))[1] > dim(unique(ssm2))[1]) {
	all.s <- rbind(sdf1, unique(ssm1), sdf2)
} else {
 	all.s <- rbind(sdf1, unique(ssm2), sdf2)
}
...
se_all.f <- we_all.f <- ns_se_all.f <- ns_we_all.f <- NULL
...
write.table(se_all.f, file=paste(parent.o, "significant_se_tes_griffin_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)

# Read the file
se_all.f <- read.table(file=paste(parent.o, "significant_se_tes_griffin_feature_gtf2.txt", sep=""), 
...
for (i in 1:length(ns_se_all[,1])) {
	...
	ns_se_all.f[i, 10] <- paste(ft, collapse=';')
}	
colnames(ns_se_all.f) <- c("Chr", "TE_ID", "Start", "Stop", "DR1","DR2","DR3","DR4","DR5", "features") 
write.table(ns_se_all.f, file=paste(parent.o, "non-significant_se_tes_griffin_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)	

# Read the file
ns_se_all.f <- read.table(file=paste(parent.o, "non-significant_se_tes_griffin_features_gtf2.txt", sep=""), 
						quote="", header=T, sep="\t", stringsAsFactors=F)
...
write.table(we_all.f, file=paste(parent.o, "significant_we_tes_griffin_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)

# Read the file
we_all.f <- read.table(file=paste(parent.o, "significant_we_tes_griffin_features_gtf2.txt", sep=""), 
...
for (i in 1:length(ns_we_all[,1])) {
	...
	ns_we_all.f[i, 10] <- paste(ft, collapse=';')
}	
colnames(ns_we_all.f) <- c("Chr", "TE_ID", "Start", "Stop", "DR1","DR2","DR3","DR4","DR5", "features") 
write.table(ns_we_all.f, file=paste(parent.o, "non-significant_we_tes_griffin_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)	

#Read the file
ns_we_all.f <- read.table(file=paste(parent.o, "non-significant_we_tes_griffin_features_gtf2.txt", sep=""), 
							quote="", header=T, sep="\t", stringsAsFactors=F)
...
ns_se_all.f.f <- length(grep("5'UTR",ns_se_all.f[,10]))
se_all.f.nf <- dim(se_all.f)[1] - length(grep("5'UTR",se_all.f[,6]))
ns_se_all.f.nf <- dim(ns_se_all.f)[1] - length(grep("5'UTR",ns_se_all.f[,10]))

se_all.f.e <- length(grep("exon",se_all.f[,6]))
ns_se_all.f.e <- length(grep("exon",ns_se_all.f[,10]))
se_all.f.ne <- dim(se_all.f)[1] - length(grep("exon",se_all.f[,6]))
ns_se_all.f.ne <- dim(ns_se_all.f)[1] - length(grep("exon",ns_se_all.f[,10]))

se_all.f.i <- length(grep("intron",se_all.f[,6]))
ns_se_all.f.i <- length(grep("intron",ns_se_all.f[,10]))
se_all.f.ni <- dim(se_all.f)[1] - length(grep("intron",se_all.f[,6]))
ns_se_all.f.ni <- dim(ns_se_all.f)[1] - length(grep("intron",ns_se_all.f[,10]))

se_all.f.t <- length(grep("3'UTR",se_all.f[,6]))
ns_se_all.f.t <- length(grep("3'UTR",ns_se_all.f[,10]))
se_all.f.nt <- dim(se_all.f)[1] - length(grep("3'UTR",se_all.f[,6]))
ns_se_all.f.nt <- dim(ns_se_all.f)[1] - length(grep("3'UTR",ns_se_all.f[,10]))

se_all.f.r <- length(grep("promoter",se_all.f[,6]))
ns_se_all.f.r <- length(grep("promoter",ns_se_all.f[,10]))
se_all.f.nr <- dim(se_all.f)[1] - length(grep("promoter",se_all.f[,6]))
ns_se_all.f.nr <- dim(ns_se_all.f)[1] - length(grep("promoter",ns_se_all.f[,10]))

se_all.f.ur <- length(grep("5UPSTR",se_all.f[,6]))
ns_se_all.f.ur <- length(grep("5UPSTR",ns_se_all.f[,10]))
se_all.f.nur <- dim(se_all.f)[1] - length(grep("5UPSTR",se_all.f[,6]))
ns_se_all.f.nur <- dim(ns_se_all.f)[1] - length(grep("5UPSTR",ns_se_all.f[,10]))

se_all.f.dr <- length(grep("3UPSTR",se_all.f[,6]))
ns_se_all.f.dr <- length(grep("3UPSTR",ns_se_all.f[,10]))
se_all.f.ndr <- dim(se_all.f)[1] - length(grep("3UPSTR",se_all.f[,6]))
ns_se_all.f.ndr <- dim(ns_se_all.f)[1] - length(grep("3UPSTR",ns_se_all.f[,10]))
...
write.table(t(c("Feature", "No all non-significant TEs", "No all significant TEs", 
			...
			file=paste(parent.o, "fisher.test_tes_se_features_griffin.txt", sep=""), 
			col.names=F, row.names=F, quote=F, sep="\t")
write.table(cbind(reg,counts,pvals, qval), file=paste(parent.o, "fisher.test_tes_se_features_griffin.txt", 
			sep=""), col.names=F, row.names=F, quote=F, sep="\t", append=T)

#### Perform the enrichment for TE families
## SE TEs
s <- read.csv(file=paste(parent.o, "significant_se_tes_griffin_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_se_tes_griffin_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
...
write.table(all.f, file=paste(parent.o, "fisher.test_se_te_families_griffin.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=T)

### Enrichment TE families (only families with => 20 TEs per family)
## SE TEs
s <- read.csv(file=paste(parent.o, "significant_se_tes_griffin_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_se_tes_griffin_features_gtf2.txt", sep=""), 
...
if (length(te.n) == 0) {
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_se_te_families_more_than_20_griffin.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
}
...
ns_we_all.f.f <- length(grep("5'UTR",ns_we_all.f[,10]))
we_all.f.nf <- dim(we_all.f)[1] - length(grep("5'UTR",we_all.f[,6]))
ns_we_all.f.nf <- dim(ns_we_all.f)[1] - length(grep("5'UTR",ns_we_all.f[,10]))

we_all.f.e <- length(grep("exon",we_all.f[,6]))
ns_we_all.f.e <- length(grep("exon",ns_we_all.f[,10]))
we_all.f.ne <- dim(we_all.f)[1] - length(grep("exon",we_all.f[,6]))
ns_we_all.f.ne <- dim(ns_we_all.f)[1] - length(grep("exon",ns_we_all.f[,10]))

we_all.f.i <- length(grep("intron",we_all.f[,6]))
ns_we_all.f.i <- length(grep("intron",ns_we_all.f[,10]))
we_all.f.ni <- dim(we_all.f)[1] - length(grep("intron",we_all.f[,6]))
ns_we_all.f.ni <- dim(ns_we_all.f)[1] - length(grep("intron",ns_we_all.f[,10]))

we_all.f.t <- length(grep("3'UTR",we_all.f[,6]))
ns_we_all.f.t <- length(grep("3'UTR",ns_we_all.f[,10]))
we_all.f.nt <- dim(we_all.f)[1] - length(grep("3'UTR",we_all.f[,6]))
ns_we_all.f.nt <- dim(ns_we_all.f)[1] - length(grep("3'UTR",ns_we_all.f[,10]))

we_all.f.r <- length(grep("promoter",we_all.f[,6]))
ns_we_all.f.r <- length(grep("promoter",ns_we_all.f[,10]))
we_all.f.nr <- dim(we_all.f)[1] - length(grep("promoter",we_all.f[,6]))
ns_we_all.f.nr <- dim(ns_we_all.f)[1] - length(grep("promoter",ns_we_all.f[,10]))

we_all.f.ur <- length(grep("5UPSTR",we_all.f[,6]))
ns_we_all.f.ur <- length(grep("5UPSTR",ns_we_all.f[,10]))
we_all.f.nur <- dim(we_all.f)[1] - length(grep("5UPSTR",we_all.f[,6]))
ns_we_all.f.nur <- dim(ns_we_all.f)[1] - length(grep("5UPSTR",ns_we_all.f[,10]))

we_all.f.dr <- length(grep("3UPSTR",we_all.f[,6]))
ns_we_all.f.dr <- length(grep("3UPSTR",ns_we_all.f[,10]))
we_all.f.ndr <- dim(we_all.f)[1] - length(grep("3UPSTR",we_all.f[,6]))
ns_we_all.f.ndr <- dim(ns_we_all.f)[1] - length(grep("3UPSTR",ns_we_all.f[,10]))
...
write.table(t(c("Feature", "No all non-significant TEs", "No all significant TEs", 
			"No non-significant TEs in feature", "No non-significant TEs in feature", "Pval", "q-val")),
			file=paste(parent.o, "fisher.test_tes_we_features_griffin.txt", sep=""), col.names=F, 
			row.names=F, quote=F, sep="\t")
write.table(cbind(reg,counts,pvals, qval), file=paste(parent.o, "fisher.test_tes_we_features_griffin.txt", sep=""), 
			col.names=F, row.names=F, quote=F, sep="\t", append=T)

#### Perform the enrichment for TE families
## WE TEs
s <- read.csv(file=paste(parent.o, "significant_we_tes_griffin_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_we_tes_griffin_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
...
all.f <- cbind(all.r, f.fisher, qval)
colnames(all.f)[5:6] <- c("p-val", "q-val")
write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_griffin_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=T)
...
if (length(te.n) == 0) {
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_griffin.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
} else {
	s.f <- ns.f <- NULL
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_more_than_20_griffin.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
}