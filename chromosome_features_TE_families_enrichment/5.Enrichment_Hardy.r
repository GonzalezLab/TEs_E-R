...
## Read files with predicted TEs (the columns are: chr, TE_ID, start, stop, SA, SB, SC; the three last columns contain the name of the selection pool if the TE is present or NA if it is absent)
...
# Find unique rows
pool1_tt <- unique(pool1_tt[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])

# TEs predicted with only TEMP
pool1_t <- read.table(file=paste(parent.f, "only_temp/", "No_TEs_1pools_or_more_f0.10_Pval.txt", sep=""), 
						header=T, stringsAsFactors=F)
pool2_t <- read.table(file=paste(parent.f, "only_temp/", "No_TEs_2pools_or_more_f0.10_Pval.txt", sep=""), 
						header=T, stringsAsFactors=F)
pool3_t <- read.table(file=paste(parent.f, "only_temp/", "No_TEs_3pools_or_more_f0.10_Pval.txt", sep=""), 
						header=T, stringsAsFactors=F)

# Find unique rows
pool1_t <- unique(pool1_t[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
...
# Find unique rows
pool1_tlex <- unique(pool1_tlex[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
...
sig_tt <- read.table(file=paste(parent.f, "significant_hardy.txt", sep=""), 
...
sig_t <- read.table(file=paste(parent.f, "only_temp/", "significant_hardy_temp.txt", sep=""), 
					header=F, stringsAsFactors=F)
...
sig_tlex <- read.table(file=paste(parent.f, "Tlex/", "significant_hardy_tlex.txt", sep=""), 
						header=F, stringsAsFactors=F)
...
# Find unique rows
pool1_tt <- unique(pool1_tt[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool2_tt <- unique(pool2_tt[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool3_tt <- unique(pool3_tt[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])

pool1_t <- unique(pool1_t[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool2_t <- unique(pool2_t[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool3_t <- unique(pool3_t[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])

pool1_tlex <- unique(pool1_tlex[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool2_tlex <- unique(pool2_tlex[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
pool3_tlex <- unique(pool3_tlex[, c("Chr", "TE_ID","Start","Stop","SA","SB","SC")])
...
# Significant TEs predicted with TEMP and TIDAL
sig_tt <- read.table(file=paste(parent.f, "significant_hardy.txt", sep=""), header=F, stringsAsFactors=F)
...
# Get the chromosome features where the significant SE TEs where located
...
write.table(se_all.f, file=paste(parent.o, "significant_se_tes_hardy_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)

# Read the file
se_all.f <- read.table(file=paste(parent.o, "significant_se_tes_hardy_features_gtf2.txt", sep=""), 
...
colnames(ns_se_all.f) <- c("Chr", "TE_ID", "Start", "Stop", ,"SA","SB","SC", "features") 
write.table(ns_se_all.f, file=paste(parent.o, "non-significant_se_tes_hardy_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)	

#Read the file
ns_se_all.f <- read.table(file=paste(parent.o, "non-significant_se_tes_hardy_features_gtf2.txt", sep=""), 
						quote="", header=T, sep="\t", stringsAsFactors=F)
...
write.table(we_all.f, file=paste(parent.o, "significant_we_tes_hardy_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)

# Read the file
we_all.f <- read.table(file=paste(parent.o, "significant_we_tes_hardy_features_gtf2.txt", sep=""), 
...
colnames(ns_we_all.f) <-c("Chr", "TE_ID", "Start", "Stop", ,"SA","SB","SC", "features") 
write.table(ns_we_all.f, file=paste(parent.o, "non-significant_we_tes_hardy_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)	

# Read the file
ns_we_all.f <- read.table(file=paste(parent.o, "non-significant_we_tes_hardy_features_gtf2.txt", sep=""), 
						quote="", header=T, sep="\t", stringsAsFactors=F)
...
write.table(t(c("Feature", "No all non-significant TEs", "No all significant TEs", 
				...
				file=paste(parent.o, "fisher.test_tes_se_features_hardy.txt", sep=""), col.names=F, row.names=F, 
				quote=F, sep="\t")
write.table(cbind(reg,counts,pvals, qval), file=paste(parent.o, "fisher.test_tes_se_features_hardy.txt", sep=""), 
...
write.table(t(c("Feature", "No all non-significant TEs", "No all significant TEs", 
			"No non-significant TEs in feature", "No non-significant TEs in feature", "Pval", "q-val")),
			file=paste(parent.o, "fisher.test_tes_we_features_hardy.txt", sep=""), col.names=F, row.names=F, quote=F, sep="\t")
write.table(cbind(reg,counts,pvals, qval), file=paste(parent.o, "fisher.test_tes_we_features_hardy.txt", sep=""), 
			col.names=F, row.names=F, quote=F, sep="\t", append=T)

#### Perform the enrichment for TE families
## SE TEs
s <- read.csv(file=paste(parent.o, "significant_se_tes_hardy_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_se_tes_hardy_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
...
write.table(all.f, file=paste(parent.o, "fisher.test_se_te_families_hardy.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=T)

## WE TEs
s <- read.csv(file=paste(parent.o, "significant_we_tes_hardy_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_we_tes_hardy_features_gtf2.txt", sep=""), 
				sep="\t", header=T, stringsAsFactor=F)
...
all.f <- cbind(all.r, f.fisher, qval)
colnames(all.f)[5:6] <- c("p-val", "q-val")
write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_hardy.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=T)
...
if (length(te.n) == 0) {
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_se_te_families_more_than_20_hardy.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
}
...
if (length(te.n) == 0) {
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_more_than_20_hardy.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
}