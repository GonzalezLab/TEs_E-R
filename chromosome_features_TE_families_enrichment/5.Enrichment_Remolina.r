...
## Read files with TEs (the columns are: chr, TE_ID, start, stop, RDS1, RDS2, RDS3; the three last columns contain the name of the selection pool if the TE is present or NA if it is absent)
...
sig_tt <- read.table(file=paste(parent.f, "significant_remolina.txt", sep=""), header=F, stringsAsFactors=F)
...
sig_t <- read.table(file=paste(parent.f, "only_temp/", "significant_remolina_temp.txt", sep=""), 
...
sig_tlex <- read.table(file=paste(parent.f, "Tlex/", "significant_remolina_tlex.txt", sep=""), 
						header=F, stringsAsFactors=F)
...
## Replace the name of the TE using the dicctionary
# One pool
...
# Find unique rows
pool1_tt <- unique(pool1_tt[, c("Chr", "TE_ID","Start","Stop","RDS1","RDS2","RDS3")])
pool1_t <- unique(pool1_t[, c("Chr", "TE_ID","Start","Stop","RDS1","RDS2","RDS3")])
pool1_tlex <- unique(pool1_tlex[, c("Chr", "TE_ID","Start","Stop","RDS1","RDS2","RDS3")])

# Get the number of pools in which TEs were predicted by TEMP + TIDAL
pool1_tt <- cbind(pool1_tt, matrix(rep("NA", dim(pool1_tt)[1]), ncol=1))
pool1_tt[,8] <- as.character(pool1_tt[,8])

for (i in 1:dim(pool1_tt)[1]) {
	if (sum(is.na(pool1_tt[i,5]), is.na(pool1_tt[i, 6]), is.na(pool1_tt[i, 7])) == 2) {
		pool1_tt[i, 8] <- 1
	}
	if (sum(is.na(pool1_tt[i,5]), is.na(pool1_tt[i, 6]), is.na(pool1_tt[i, 7])) == 1) {
		pool1_tt[i, 8] <- 2
	}
	if (sum(!is.na(pool1_tt[i,5]), !is.na(pool1_tt[i, 6]), !is.na(pool1_tt[i, 7])) == 3) {
		pool1_tt[i, 8] <- 3
	}
		
}
	
# Get the number of pools in which TEs were predicted by Temp
pool1_t <- cbind(pool1_t, matrix(rep("NA", dim(pool1_t)[1]), ncol=1))
pool1_t[,8] <- as.character(pool1_t[,8])

for (i in 1:dim(pool1_t)[1]) {
	if (sum(is.na(pool1_t[i,5]), is.na(pool1_t[i, 6]), is.na(pool1_t[i, 7])) == 2) {
		pool1_t[i, 8] <- 1
	}
	if (sum(is.na(pool1_t[i,5]), is.na(pool1_t[i, 6]), is.na(pool1_t[i, 7])) == 1) {
		pool1_t[i, 8] <- 2
	}
	if (sum(!is.na(pool1_t[i,5]), !is.na(pool1_t[i, 6]), !is.na(pool1_t[i, 7])) == 3) {
		pool1_t[i, 8] <- 3
	}
}

# Get the number of pools in which TEs were predicted by Tlex 
pool1_tlex <- cbind(pool1_tlex, matrix(rep("NA", dim(pool1_tlex)[1]), ncol=1))
pool1_tlex[,8] <- as.character(pool1_tlex[,8])

for (i in 1:dim(pool1_tlex)[1]) {
	if (sum(is.na(pool1_tlex[i,5]), is.na(pool1_tlex[i, 6]), is.na(pool1_tlex[i, 7])) == 2) {
		pool1_tlex[i, 8] <- 1
	}
	if (sum(is.na(pool1_tlex[i,5]), is.na(pool1_tlex[i, 6]), is.na(pool1_tlex[i, 7])) == 1) {
		pool1_tlex[i, 8] <- 2
	}
	if (sum(!is.na(pool1_tlex[i,5]), !is.na(pool1_tlex[i, 6]), !is.na(pool1_tlex[i, 7])) == 3) {
		pool1_tlex[i, 8] <- 3
	}
}
...
colnames(sm1)[8] <- "No_Pools"
colnames(df1)[8] <- "No_Pools"
...
colnames(sm2)[8] <- "No_Pools"
colnames(df2)[8] <- "No_Pools"
...
# Remove redundancy between those detected by two methods and those detected only with temp
sdf3 <- ssm3 <- NULL
for (i in 1:length(sig_tt[,1])) {
	c <- 0
	for (j in 1:length(sig_t[,1])) {
		if ( sig_tt[i,1] == sig_t[j,1] & sig_tt[i,2] == sig_t[j,2] ) {
			if (c(sig_tt[i,3], sig_tt[i,4]) %overlaps% c(sig_t[j,3], sig_t[j,4]) == TRUE) {	
				c <- c + 1
			}
		}
	}
	if (c == 0) {
		sdf3 <- rbind(sdf3, sig_tt[i, ])
	} else { 
		ssm3 <- rbind(ssm3, sig_tt[i, ]) 
	}
}			
...
for (i in 1:length(sig_t[,1])) {
	c <- 0
	for (j in 1:length(sig_tt[,1])) {
		if ( sig_t[i,1] == sig_tt[j,1] & sig_t[i,2] == sig_tt[j,2] ) {
			if (c(sig_t[i,3], sig_t[i,4]) %overlaps% c(sig_tt[j,3], sig_tt[j,4]) == TRUE) {	
				c <- c + 1
			}
		}
	}
	if (c == 0) {
		sdf4 <- rbind(sdf4, sig_t[i, ])
	} else { 
		ssm4 <- rbind(ssm4, sig_t[i, ]) 
	}
}	

# Check that inverting the comparison the results are the same
if (dim(ssm3)[1] == dim(ssm4)[1]) {
	nr <- NULL
	nr <- rbind(sdf3,ssm3,sdf4) 
} else { print("Possible Error") }
		  
sig1_t <- sdf4[sdf4[ ,5] == 1, ]
sig_many_t <- sdf4[sdf4[ ,5] > 1, ]
...
we_all <- ns_we_all <- NULL
we_all <- rbind(sig1_tt, sig1_t, sig_many_t, sig1_tlex)
colnames(pool1_tlex)[8] <- "No_Pools"
ns_we_all <- rbind(all, pool1_tlex[pool1_tlex[,8] == 1, 1:8])
...
write.table(we_all.f, file=paste(parent.o, "significant_we_tes_remolina_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)

# Read the file 
we_all.f <- read.table(file=paste(parent.o, "significant_we_tes_remolina_features_gtf2.txt", sep=""), 
...
write.table(ns_we_all.f, file=paste(parent.o, "non-significant_we_tes_remolina_features_gtf2.txt", sep=""), 
			sep="\t", quote=F, col.names=T, row.names=F)	

# Read the file
ns_we_all.f <- read.table(file=paste(pth, "non-significant_we_tes_remolina_features_gtf2.txt", sep=""), 
...
write.table(t(c("Feature", "No all non-significant TEs", "No all significant TEs", 
	...
	file=paste(parent.o, "fisher.test_tes_we_features_remolina.txt", sep=""), col.names=F, row.names=F, 
				quote=F, sep="\t")
write.table(cbind(reg,counts,pvals, qval), file=paste(pth, "fisher.test_tes_we_features_remolina.txt", 
			sep=""), col.names=F, row.names=F, quote=F, sep="\t", append=T)

#### Perform the enrichment for TE families
# WE TEs
s <- read.csv(file=paste(parent.o, "significant_we_tes_remolina_features_gtf2.txt", sep=""), 
					sep="\t", header=T, stringsAsFactor=F)
ns <- read.csv(file=paste(parent.o, "non-significant_we_tes_remolina_features_gtf2.txt", sep=""), 
					sep="\t", header=T, stringsAsFactor=F)
...
write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_remolina.txt", sep=""), sep="\t", quote=F, col.names=T, row.names=T)
...
if (length(te.n) == 0) {
	...
	f.fisher <- NULL
	for (i in 1:length(all.r[,1])) {
		f.fisher <- c(f.fisher, fisher.test(matrix(all.r[i,], ncol = 2, nrow = 2, byrow=T), 
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(pth, "fisher.test_we_te_families_remolina_gtf2_alt.txt", sep=""), 
	...
	all.f <- cbind(all.r, f.fisher, qval)
	colnames(all.f)[5:6] <- c("p-val", "q-val")
	write.table(all.f, file=paste(parent.o, "fisher.test_we_te_families_more_than_20_remolina.txt", sep=""), 
				sep="\t", quote=F, col.names=T, row.names=T)
}